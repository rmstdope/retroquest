...
class BarmaidElena(Character):
    ...
    def talk_to(self, game_state: GameState) -> str:
        if game_state.get_story_flag(FLAG_INNKEEPERS_DAUGHTER_COMPLETED):
            return (
                f"[character_name]{self.get_name()}[/character_name]: *beaming with health and vitality* "
            ...
            return (
                f"[character_name]{self.get_name()}[/character_name]: *glowing with pure energy* "
            ...
            return (
                f"[character_name]{self.get_name()}[/character_name]: *looking much better but still "
                "weak* The healing magic has given me strength! I can feel the curse's "
                "grip weakening, but it's still there, clinging to me. I think you'll need "
                "to purify the dark magic with blessed water and then cast a powerful "
                "dispelling spell to break it completely."
            )
        elif not game_state.get_story_flag(FLAG_KNOWS_ELENA_CURSE):
            game_state.set_story_flag(FLAG_KNOWS_ELENA_CURSE, True)
            return (
                f"[character_name]{self.get_name()}[/character_name]: *coughs weakly* Hello, traveler. "
    ...
    def receive_greater_heal(self) -> str:
        ...
        return (
            f"[success]You cast [spell_name]{greater_heal_spell.get_name()}[/spell_name] on Elena, "
            "channeling powerful healing energy into her weakened form. The golden light "
            "battles against the dark tendrils of the curse, and while it cannot break the "
            "enchantment completely, it significantly strengthens Elena's life force. "
            "She gasps as color returns to her cheeks and she stands a little straighter.\n\n"
            "The healing has made her strong enough to endure the final steps of the cure, "
            "but the curse still clings to her. Now you'll need to purify her and dispel "
            "the curse.[/success]"
        )
    ...
    def receive_crystal_water_purification(
        ...
        return (
            "[success]You carefully pour the [item_name]crystal-clear water[/item_name] over Elena. "
            "The blessed water sparkles with divine light as it touches her skin, and dark "
            "shadows begin to steam away like smoke. Elena gasps as the corrupting influence "
            "of the curse is washed from her body and soul. The water glows brighter, "
            "purifying every drop of dark magic it touches.\n\n"
            "As the last of the water drips away, Elena stands straighter, her skin glowing "
            "with a faint, pure radiance. The curse's hold on her has been greatly weakened "
            "by the purification. Now only a powerful dispelling spell remains to shatter "
            "the last remnants of the dark enchantment.[/success]"
        )
    ...
    def receive_dispel_final_cure(
        ...
        return (
            f"[success]You cast [spell_name]{dispel_spell.get_name()}[/spell_name] on Elena, "
            "channeling powerful counter-magic into the remaining curse fragments. The "
            "spell tears through the last dark tendrils like lightning, shattering the "
            "evil enchantment completely. Elena cries out in relief as the final traces "
            "of the curse dissolve into nothingness.\n\n"
            "She stands tall and straight, her skin glowing with perfect health. The "
            "curse that has tormented her for weeks is finally broken! Elena looks at you "
            "with tears of joy, completely free at last. The three-step healing process - "
            "greater heal, purification, and dispelling - has worked perfectly to save her "
            "life.[/success]"
        )
