...
class WaterNymphs(Character):
    ...
    def talk_to(self, game_state: GameState) -> str:
        ...
        if self.riddles_completed:
            ...
            return (event_msg + "\n" +
                   ...
                   f"Use the command 'answer [your response]' to give your solution.[/info]")
        else:
    ...
    def answer_riddle(self, game_state: GameState, answer: str) -> str:
        """Handle riddle answers from the player."""
        if self.riddles_completed:
            return f"[info]The [character_name]{self.get_name()}[/character_name] have already accepted your wisdom.[/info]"
        ...
        if answer_lower == riddle["answer"]:
            ...
            if self.current_riddle >= len(self.riddles):
                ...
                return (f"[success]Correct! {riddle['explanation']}[/success]\n\n"
                       f"[dialogue]The [character_name]{self.get_name()}[/character_name] sing "
                       f"in harmonious delight. 'Wisely answered, all three riddles! You have "
                ...
                return (f"[success]Correct! {riddle['explanation']}[/success]\n\n"
                       ...
                       f"approvingly. 'Well answered! Now for the {self.current_riddle + 1} riddle:'[/dialogue]\n\n"
                       
                       f"[dialogue]'{next_riddle['question']}'[/dialogue]")
        else:
            # Incorrect answer
            return (f"[failure]The [character_name]{self.get_name()}[/character_name] shake "
                   f"their heads gently. 'Not quite, young seeker. Think more deeply about "
