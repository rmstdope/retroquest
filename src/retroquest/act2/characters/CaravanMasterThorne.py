    ...
    def talk_to(self, game_state: GameState) -> str:
        if game_state.get_story_flag(FLAG_FOUND_LOST_CARAVAN):
            if not self.has_given_documents:
                ...
                prefix = f"[character_name]{self.get_name()}[/character_name]: [dialogue]You've found them! "
                middle = (
                ...
                return prefix + middle + (
                    f"Thorne hands you a stack of [item_name]{secret_docs.get_name()}[/item_name].[/dialogue]"
                )
            else:
                return (
                    f"[character_name]{self.get_name()}[/character_name]: [dialogue]I'm still processing "
                    ...
                    "can restore honor where it was unjustly lost.[/dialogue]"
                )
            ...
            return (
                f"[character_name]{self.get_name()}[/character_name]: [dialogue]Thank the gods, someone "
                ...
                "Would you be willing to search for them? I would be forever in your debt.[/dialogue]"
            )
        else:
            return (
                f"[character_name]{self.get_name()}[/character_name]: [dialogue]Any word on my missing "
