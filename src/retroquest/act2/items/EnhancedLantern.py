...
class EnhancedLantern(Item):
    def __init__(self) -> None:
        super().__init__(
            ...
            short_name="lantern",
            description=(
                "A magically enhanced lantern that burns with bright, steady light. The enchantment "
                "allows it to illuminate hidden paths and reveal magical auras that would otherwise "
                "remain invisible."
            ),
            can_be_carried=True,
        )
    ...
    def use(self, game_state: GameState) -> str:
        ...
        if isinstance(game_state.current_room, ForestEntrance):
            ...
            "ethereal blue light that reveals hidden paths through the dense undergrowth. The "
            "light seems to resonate with the forest's natural magic, illuminating safe routes and "
            "warning you away from dangerous areas. With this enhanced vision, you can navigate "
            "the forest with confidence.[/item_effect]")
            ...
            return (
                "You activate the enhanced lantern. Its magical light pierces through darkness and can "
                "reveal hidden passages, magical traces, and secret paths that ordinary light cannot "
                "illuminate."
            )