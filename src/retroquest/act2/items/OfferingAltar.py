...
class OfferingAltar(Item):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="offering altar",
            description="An ancient altar carved from a single piece of starstone, its surface adorned with mystical "
                        "runes that pulse with otherworldly light. Three circular indentations suggest that specific "
                        "items are meant to be placed here in a sacred ritual. The altar radiates powerful magic and "
                        "seems to be waiting for the proper offerings to activate its full potential."
        )
    ...
    def use_with(self, game_state: GameState, other_item) -> str:
        ...
        if not isinstance(other_item, (DruidicCharm, ProtectiveCharm, NaturesCharm)):
            return (f"The altar's runes remain dim. The {other_item.name} does not seem to be one of the sacred "
                    f"charms needed for this ancient ritual. You need the three blessed charms: Druidic Charm, "
                    f"Protective Charm, and Nature's Charm.")
        ...
        if not (has_druidic and has_protective and has_natures):
            ...
            return (f"The altar's runes flicker weakly as you place the {other_item.name} upon it. You sense that "
                    f"you need all three sacred charms to activate the full ritual: {', '.join(missing_charms)}. "
                    f"The ancient magic requires all three offerings to complete the summoning.")
        ...
        summon_message = ("As the three sacred charms are placed upon the altar, the starstone erupts in "
