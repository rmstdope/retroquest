...
class TrainingSword(Item):
    ...
    def use(self, game_state: GameState) -> str:
        ...
        if (
            ...
            and any(isinstance(char, SirCedric) for char in game_state.current_room.get_characters())
        ):
            if not game_state.get_story_flag(FLAG_DEMONSTRATED_COMBAT_SKILLS):
                ...
                return (
                    "[success]You draw the training sword and perform a series of combat forms for "
                    "[character_name]Sir Cedric[/character_name]. Your movements are fluid and "
                    "precise, demonstrating skill with both offensive and defensive techniques.\n\n"

                    "Sir Cedric nods approvingly. 'Excellent! Your martial training is evident. I "
                    "can trust someone with such disciplined combat skills.'\n\n"
                    ...
                    "the dark woods, and quality rope for navigating difficult terrain. Only when "
                    "you're properly equipped should you venture into those mystical lands to "
                    "seek out Nyx.'\n\n"

                    "He straightens up, his voice returning to its commanding tone. 'Time is of "
                    "the essence, Elior. The darkness grows stronger each day, and we need Nyx's "
                    "ancient wisdom to understand what we're truly facing.'[/success]"
                )

            elif game_state.get_story_flag(FLAG_DEMONSTRATED_COMBAT_SKILLS):
                # Subsequent uses after mission explanation
                return (
                    "[success]You practice a few more combat forms with the training sword, and "
                    ...
                    "those supplies from the Market District first - a forest survival kit, "
                    "enhanced lantern, and quality rope. The mystical being Nyx holds ancient "
                    "knowledge that could be crucial in our fight against the gathering darkness.'\n\n"

                    "He looks toward the forest beyond the city walls. 'The enchanted forest is "
                    "dangerous, but with proper preparation and your proven combat skills, you "
                    "may succeed where others have failed. Nyx's wisdom could be the key to "
            ...
            return (
                "You practice a few sword forms with the training sword. The balance feels good in "
                "your hands, and it would be perfect for demonstrating combat skills to someone who "
                "needs proof of your abilities."
            )
