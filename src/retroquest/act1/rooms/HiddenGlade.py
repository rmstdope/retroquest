...
class HiddenGlade(Room):
    ...
    def get_room_text_description(self) -> str:
        ...
        if not self.light_spell_cast:
            base_description += (
                " Yet, an unnatural darkness lingers here, as if the glade itself is waiting to be "
                "enlightened."
            )
        dynamic_parts = []
    ...
    def rest(self, game_state: GameState) -> str:
        if game_state.get_story_flag("deer_can_be_observed"):
            ...
            if not self.get_character_by_name("Deer"):
                ...
                messages.append(
                    "As you relax, a graceful [character_name]Deer[/character_name] emerges into the "
                    "glade. It gazes at you with deep, ancient eyes, and for a moment, you feel a "
                    "surge of gentle magical energy flow through your body, as if the glade itself "
                    "is welcoming you."
                )
                self.add_item(RareFlower())
                messages.append(
                    "You notice a rare, beautiful [item_name]Rare Flower[/item_name] blooming near "
                    "the mossy stone."
                )
            else:
            ...
            return (
                "You rest for a while, but the glade remains quiet. Perhaps something more needs to "
                "happen for its magic to awaken."
            )
    ...
    def light(self, game_state: GameState) -> str:
        ...
        message = (
            "[event]You cast a [spell_name]light[/spell_name] spell that illuminates the glade.[/event]\n"
            "The inscription on the mossy stone glows faintly, revealing ancient runes. They speak of a "
            "seed, a song, and the sun's embrace to awaken life."
        )
