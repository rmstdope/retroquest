...
class Locket(Item):
    ...
    def picked_up(self, game_state: GameState) -> str | None:
        ...
        if not game_state.has_spell(BlessSpell().name):
            ...
            return (f"[event]You pick up the locket.[/event]\nAs you do this, the [character.name]priest[/character.name] gasps. The [character.name]priest[/character.name] nods gravely. [dialogue]'That locket... it is a relic of the village founders. It speaks of your connection to this place and its history. May this blessing protect you on your path.'[/dialogue]\n "
                    f"He teaches you the [spell.name]bless[/spell.name] spell!\n[event]You have learned [spell.name]bless[/spell.name].[/event]")
