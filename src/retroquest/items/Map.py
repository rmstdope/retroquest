...
class Map(Item):
    ...
    def use(self, game_state: GameState) -> str:
        ...
        if game_state.current_room.name.lower() == "road to greendale":
            ...
            return f"The [item.name]{self.get_name()}[/item.name] aligns with the landscape, revealing a hidden path that shortens the journey to Greendale. You feel a sense of accomplishment as you set forth. (Act I Completed)"
        return f"You study the [item.name]{self.get_name()}[/item.name]. It depicts the local area with surprising detail."
