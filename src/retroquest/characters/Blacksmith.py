...
class Blacksmith(Character):
    ...
    def talk_to(self, game_state) -> str:
        ...
        if game_state.get_story_flag("blacksmith_met"):
            return "[dialogue]'Welcome back, young [character.name]Elior[/character.name]. Need something else?'[/dialogue]"
        else:
            game_state.set_story_flag("blacksmith_met", True)
            return f"[dialogue]'Well met, young one. I am Borin, the village [character.name]{self.get_name()}[/character.name]. If you need tools made or mended, I'm your man. A sharp blade can be a good friend.'[/dialogue]"
    ...
    def give_item(self, game_state, item_object) -> str:
        if isinstance(item_object, Coin):
            ...
            if dull_knife_instance:
                ...
                return f"The [character.name]{self.get_name()}[/character.name] takes your [item.name]{item_object.get_name()}[/item.name] and your [item.name]{dull_knife_instance.get_name()}[/item.name]. He expertly sharpens it on his whetstone and hands you back a gleaming [item.name]sharp knife[/item.name]!"
            else:
                return f"The [character.name]{self.get_name()}[/character.name] eyes the [item.name]{item_object.get_name()}[/item.name]. [dialogue]'Thanks for the offer, but I can only sharpen a [item.name]dull knife[/item.name] for ye if you have one.'[/dialogue]"
