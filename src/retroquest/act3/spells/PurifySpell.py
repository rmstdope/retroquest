...
class PurifySpell(Spell):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="purify",
            description=(
                "A cleansing rite tuned to salt and stone; it scours sea-grit and wakes dormant ward-glyphs."
            ),
        )
    ...
    def cast_on_item(self, game_state: GameState, target_item: Item) -> str:
        ...
        return (
            f"[event]You cast [spell_name]{self.get_name()}[/spell_name] on [item_name]{target_item.get_name()}[/item_name].[/event]\n"
    ...
    def cast_on_character(self, game_state: GameState, target_character: Character) -> str:
        return (
            f"[event]You cast [spell_name]{self.get_name()}[/spell_name] on [character_name]{target_character.get_name()}[/character_name].[/event]\n"
