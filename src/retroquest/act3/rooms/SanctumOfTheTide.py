...
class TideBornGuardian(Character):
    def __init__(self) -> None:
        super().__init__(
            ...
            description=(
                "A figure of gathered waters and ward-sigils, its form rippling with the tide's breath."
            ),
        )

    def talk_to(self, game_state: GameState) -> str:
        return (
            "[dialogue]The guardian's voice is the hush of a turning tide: 'Name what you will not abandon.'[/dialogue]"
        )
    ...
    def say_to(self, words: str, game_state: GameState) -> str:
        ...
        if w == "myself":
            ...
            if not (game_state.get_story_flag(FLAG_ACT3_TIDEWARD_SIGILS_ATTUNED) and game_state.get_story_flag(FLAG_ACT3_LANTERNS_OF_THE_DEEPS_LIT)):
                return (
                    "[failure]The guardian stirs, but the chamber's wards resist. The outer rites remain unfinished." 
                    " Complete the sigils and light the path before the waters will part.[/failure]"
                )
            game_state.set_story_flag(FLAG_ACT3_VOW_OF_COURAGE_MADE, True)
            return (
                "[success]The waters draw back as if breathing in. Your vow anchors the chamber's heart, and the relic heeds your courage." 
                " You may now take the [item_name]Crystal of Light[/item_name].[/success]"
            )
        return (
            "[dialogue]The tide listens, then falls quiet. That is not the vow the guardian will bind to.[/dialogue]"
        )
