...
class LanternsOfTheDeepsQuest(Quest):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            description=(
                "Retrieve three prism lanterns from the pier vault and mount them along the submerged approach."
                " Kindle them to reveal the safe path to the sanctum."
            ),
            completion=(
                "The prism lanterns burn with steady, cold flame. The submerged path is revealed — the sanctum lies ahead."
            ),
        )

    def check_trigger(self, game_state: GameState) -> bool:
        # Trigger when entering Submerged Antechamber or Collapsed Pier, but only after Tideward Sigils complete.
